#!/usr/bin/env python3
"""Canonical-lane closure guard for the smooth 4d poincare conjecture workspace."""
...
def compute_report(data: dict[str, Any], sigma_star: float | None, strict_coh_zero: bool) -> dict[str, Any]:
    ...
    native_names = ['S4P_G1', 'S4P_G2', 'S4P_G3', 'S4P_G4', 'S4P_G5']
    gates = {name: ('PASS' if passed else 'FAIL') for name, passed in zip(native_names, gate_pass)}
    gates['S4P_G6'] = 'PASS' if coh_pass else 'FAIL'
    gates['S4P_GM'] = 'PASS' if gm_pass else 'FAIL'
    ...
    blockers['S4P_G6'] = [] if coh_pass else ['eps_coh missing/invalid or strict-zero target failed']
    blockers['S4P_GM'] = [] if gm_pass else ['strict margin <= 0 or upstream gates failed']
    ...
    report = {
        'meta': {
            ...
            'framework': 'smooth_4d_poincare_conjecture',
        },
        'schema': {
